...
`composefs-1-16-bridge` is historical-client coverage, restricted to the
bootc 1.16.0 old-stager fixture. It is disabled by default in the plan
(enabled by `--context composefs_bridge=true`). It requires a read-only
shared container store and prebuilt fixture images; the test itself does
not build, copy, or SCP images. `just test-composefs-bridge sealed` (or `unsealed`) builds the
fixtures from the current tree and runs it; CI does this on Fedora 44. The
stager is the current image with bootc replaced by the pinned 1.16.0 RPM and
its initramfs and UKI regenerated by that version
(`tmt/tests/Dockerfile.composefs-1-16-stager`).

The covered path is an old-stager migration where the old client is followed
by current bootc and a newly generated initramfs. bootc 1.16.0 stages the V2
fallback of the dual-digest UKI; 1.16.4 and later clients would stage V1
directly, which this test does not cover.
Both sealed and unsealed UKIs are covered; to run one case against fixtures
...
strict fixture. The caller (or `just test-composefs-bridge`) supplies the
matching pullspecs through the variables above.
...
installs. The test is disabled by default in the plan (enabled by
`--context composefs_bridge=true`), so the regular test-integration matrix
doesn't build these large fixtures; CI runs it in its own job.
