...
**bootc-finalize-staged-hold.service**, which keeps `/boot` open while
a deployment is staged. Otherwise an automounted `/boot` (such as the
ESP set up by **systemd-gpt-auto-generator**(8)) could expire while
idle, and then either deadlock with shutdown or be unavailable when
finalization runs.
